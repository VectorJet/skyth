...
class MusicLibraryTool(BaseTool):
    ...
    def execute(self, **kwargs: Any) -> Any:
        try:
            ...
            print(f"[Music Library] Found {len(songs)} songs, total audio data: {total_audio_size / (1024*1024):.2f} MB")
            ...
            if audio is not None:
                ...
                if tags:
                    ...
                    if 'TIT2' in tags:  # MP3
                        ...
                        title = str(tags['\xa9nam'][0]) if isinstance(tags['\xa9nam'], list) else str(tags['\xa9nam'])
                    else:
                    ...
                    if 'TPE1' in tags:  # MP3
                        ...
                        artist = str(tags['\xa9ART'][0]) if isinstance(tags['\xa9ART'], list) else str(tags['\xa9ART'])
                    ...
                    try:
                        ...
                        print(f"[Music Library] Could not extract album art from {filename}: {art_error}")
            else:
                title = os.path.splitext(filename)[0]
