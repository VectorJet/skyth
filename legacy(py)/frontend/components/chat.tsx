...
const Chat = () => {
	...
	const [maximizedImageUrl, setMaximizedImageUrl] = useState<string | null>(
		null,
	);
	...
	const handleEditSubmit = (params: {
		...
		const parentMessageId =
			msgIndex > 0
				? parseInt(String(messages[msgIndex - 1].id), 10)
				: undefined;
		submitQuery({ userInput, editInfo, parentMessageId });
	};

	const handleRegenerateSubmit = (params: {
		regenInfo: { group_uuid: string };
	}) => {
		const { regenInfo } = params;
		const msgIndex = messages.findIndex(
			(m) => m.message_group_uuid === regenInfo.group_uuid,
		);
		...
		const parentMessageId =
			msgIndex > 0
				...
				: undefined;
		const userInput =
			msgIndex > 0 ? messages[msgIndex - 1].content : "Regenerate response";
	...
	useEffect(() => {
		if (!isUserScrollingRef.current) scrollToBottom("smooth");
	}, [messages.length, liveMessage?.content]);

	return (
		<div className="relative flex flex-col flex-grow min-h-0 h-full">
			{maximizedImageUrl && (
				<ImageModal src={maximizedImageUrl} alt="Maximized image" />
			)}
			...
			{!isInitial && (
				...
				<div
					ref={chatAreaRef}
					className="flex-grow overflow-y-auto overflow-x-hidden px-5 min-h-0 pb-[240px]"
				>
			...
			{showScrollButton && (
				<button
					...
					className="fixed bottom-[140px] left-1/2 -translate-x-1/2 z-[25] w-8 h-8 min-h-0 rounded-full bg-surface border border-border-color text-primary-text flex items-center justify-center flex-shrink-0 cursor-pointer transition-all hover:bg-button-bg hover:scale-110 shadow-xl aspect-square"
				>
