# backend/apps/wikipedia/tools/wikipedia_tool.py
import wikipediaapi
import traceback
import requests
...
class WikipediaTool(BaseTool):
        ...
        return [
            {
                "name": "query",
                "type": "string",
                "description": "The search term or title of the Wikipedia page.",
            }
        ]
    ...
    def _get_page_image_url(self, page_title: str) -> str | None:
        """Makes a direct request to the MediaWiki API to get the main page image."""
        try:
            session = requests.Session()
            # --- CRITICAL FIX: Add a User-Agent header to the request ---
            headers = {
            ...
            print(
                f"🟡 [WikipediaTool] Could not fetch image for '{page_title}' via MediaWiki API: {e}"
            )
            return None
    ...
    def execute(self, query: str, **kwargs: Any) -> Any:
        try:
            ...
            wiki_wiki = wikipediaapi.Wikipedia(
                language="en", user_agent="Skyth-AI-Assistant/1.0"
            )
            ...
            if not full_text:
                print(
                    f"🟡 [WikipediaTool] Page '{query}' found, but no text content could be extracted."
                )
                return {
                    "error": f"The Wikipedia page for '{query}' exists but contains no readable content."
                }

            print(
                f"✅ [WikipediaTool] Successfully fetched data for '{query}'. Image URL: {image_url}"
            )
            return {
            ...
            print(
                f"🔴 [WikipediaTool] An unexpected error occurred while searching for '{query}': {e}"
            )
            traceback.print_exc()
            return {
                "error": "An unexpected server error occurred while searching Wikipedia."
            }
