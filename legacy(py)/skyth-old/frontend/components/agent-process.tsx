...
const AgentProcess = ({ steps, isLoading }: AgentProcessProps) => {
  ...
  const statusText = isLoading 
    ? (currentStep?.type === 'thought' ? 'Thinking...' : currentStep?.type === 'tool_call' ? `Using ${currentStep.tool}...` : 'Processing...')
    : 'Completed';
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col items-start">
            ...
            <span className="text-xs text-secondary-text">{statusText}</span>
          </div>
        </div>
