...
import { Copy, Check, Pen, RotateCw, ChevronLeft, ChevronRight } from 'lucide-react';
...
const UserMessageContent = ({ content, connectedApps }: { content: string, connectedApps: App[] }) => {
    ...
    if (app) {
      return (
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="prose prose-invert max-w-none prose-p:my-0" dangerouslySetInnerHTML={{ __html: marked.parse(restOfContent) }} />
        </div>
      );
    }
  }

  return <div className="prose prose-invert max-w-none prose-p:my-0" dangerouslySetInnerHTML={{ __html: marked.parse(content) }} />;
};

const MessageEdit = ({ message, onSave, onCancel }: { message: Message, onSave: (newContent: string) => void, onCancel: () => void }) => {
  ...
  return (
    <div className="w-full bg-button-bg text-primary-text py-3 px-[18px] rounded-3xl rounded-br-lg animate-slide-fade-in">
      ...
      <div className="flex justify-end gap-2 mt-2">
        <button onClick={onCancel} className="px-3 py-1 text-sm rounded-md hover:bg-black/20">Cancel</button>
        <button onClick={handleSave} className="px-4 py-1.5 text-sm bg-accent text-white rounded-full hover:opacity-90 transition-opacity">Save & Submit</button>
      </div>
    </div>
  );
};
...
const ChatMessageComponent = ({ 
  ...
  connectedApps: App[],
  onEditSubmit?: (params: { userInput: string; editInfo: { group_uuid: string; old_message_id: string; }; }) => void;
  ...
  const handleEditSave = (newContent: string) => {
    if (onEditSubmit) {
      onEditSubmit({
        ...
        editInfo: { group_uuid: message.message_group_uuid, old_message_id: message.id }
      });
    }
    setIsEditing(false);
  };
  ...
  const handleSwitchVersion = (direction: 'prev' | 'next') => {
    ...
    const targetId = direction === 'prev' ? message.version_info.prev_id : message.version_info.next_id;
    if (targetId) {
  ...
  const canShowControls = !finalIsLoading && (
  ...
  return (
    ...
    >
      {isUser ? (
        <div className={`w-full flex flex-col items-end ${uniqueArtifacts.length > 0 ? 'gap-2' : ''}`}>
            ...
            <div className="max-w-[85%] md:max-w-[80%] w-full flex justify-end">
              <div className="flex flex-row gap-2 flex-wrap justify-end">
                {uniqueArtifacts.map((artifact, index) => (
                  <div key={`user-artifact-${message.id}-${index}`} className="w-20 h-20">
                    <ArtifactRenderer artifact={artifact} onWidgetAction={onWidgetAction} onImageMaximize={onImageMaximize} />
                  </div>
                ))}
              </div>
            </div>
          ...
          {(message.content || isEditing) && (
            <div className="w-full flex justify-end">
              {isEditing ? (
                <MessageEdit message={message} onSave={handleEditSave} onCancel={() => setIsEditing(false)} />
              ) : (
                ...
                <div className="max-w-[85%] md:max-w-[80%] bg-button-bg text-primary-text py-3 px-[18px] rounded-3xl rounded-br-lg animate-slide-fade-in">
                  <CollapsibleContent maxHeight={300}>
                    <UserMessageContent content={message.content} connectedApps={connectedApps} />
                  </CollapsibleContent>
                </div>
              )}
            </div>
          ...
          <div className="max-w-full self-start py-2.5 w-full">
            <div className="flex flex-col gap-1">
              {message.initialContent && (
                 <StreamingText content={message.initialContent} isStreaming={finalIsLoading && !message.agentCall} />
              )}
              {message.agentCall && <AgentCallDisplay agentCall={message.agentCall} />}
              {AgentComponent && isAppInteraction && (
                <AppInteraction 
                  agentCall={message.agentCall}
                  isLoading={finalIsLoading && uniqueArtifacts.length === 0} 
                />
              )}
              {AgentComponent && !isAppInteraction && (hasAgentSteps || finalIsLoading) && (
                <AgentComponent 
              ...
              {uniqueArtifacts.length > 0 && (
                <div className="flex flex-col gap-2 mt-2">
                  {uniqueArtifacts.map((artifact, index) => {
                     const key = `${artifact.type}-${(artifact as any).id || JSON.stringify(artifact)}`;
                    return <ArtifactRenderer key={key} artifact={artifact} onWidgetAction={onWidgetAction} onImageMaximize={onImageMaximize} />;
                  })}
                </div>
              )}
              {message.content && (
                 <StreamingText content={message.content} isStreaming={finalIsLoading} />
              )}
              {finalIsLoading && !message.initialContent && !message.content && !message.agentCall && !hasArtifacts && (
      ...
      <div 
        className={`w-full flex ${isUser ? 'justify-end' : 'justify-start'} overflow-hidden transition-all duration-200 ease-out ${
            isHovered && canShowControls ? 'max-h-8 opacity-100 mt-1.5' : 'max-h-0 opacity-0 mt-0'
        }`}
      >
        <div className="flex items-center gap-1 px-2 py-1 rounded-lg bg-surface border border-border-color">
          ...
          {message.content && (
            <button onClick={handleCopy} className="p-1 rounded hover:bg-button-bg">
              {isCopied ? (
          ...
          {isUser && onEditSubmit && !isEditing && (
            <button onClick={() => setIsEditing(true)} className="p-1 rounded hover:bg-button-bg">
              <Pen className="w-3.5 h-3.5" />
            </button>
          )}
          
          {!isUser && onRegenerateSubmit && (
            <button onClick={handleRegenerate} className="p-1 rounded hover:bg-button-bg">
