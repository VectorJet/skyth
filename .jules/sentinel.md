...
## 2024-05-30 - Path Traversal / Directory Access Bypass in filesystem tools
**Vulnerability:** The `list_dir_tool.ts` and `edit_file_tool.ts` tools failed to enforce the `workspaceOnly` configuration setting for filesystems, allowing users to potentially list directory contents or edit files outside of the allowed workspace by using absolute paths or path traversal (e.g. `../`).
**Learning:** Security checks like `workspaceOnly` were implemented in some filesystem tools (`read_file`, `write_file`) but missed in others (`list_dir`, `edit_file`). This creates a vulnerability where defense mechanisms are inconsistently applied across a category of tools.
**Prevention:** Always implement centralized security checks or thoroughly verify that all related tool components implement the necessary access checks and restrictions, especially concerning filesystem and path boundaries.
