...
function computeHashHex(salt: Buffer, value: string): string {
	return createHash("sha256").update(salt).update(value, "utf-8").digest("hex");
}
...
export function readLatestSecretValue(params: {
	...
	for (let i = lines.length - 1; i >= 0; i -= 1) {
		...
		if (
			...
			record.key_path !== params.keyPath
		)
			continue;
		...
		try {
			...
			const ciphertext = Buffer.from(
				record.encryption.ciphertext_b64,
				"base64",
			);
...
export function secretFileDirectory(
	scope: string,
	overrideAuthDir?: string,
): string {
