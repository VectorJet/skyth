...
export async function rotateTokenCommandHandler(
	args: string[],
	...
	const password = await promptInput(
		"Enter superuser password to rotate token: ",
	);
	if (!password) {
	...
	try {
		...
		console.log("");
		console.log(
			"IMPORTANT: Any external systems using the old token will need to be updated.",
		);

		return 0;
	} catch (error) {
		console.error(
			`Error rotating token: ${error instanceof Error ? error.message : error}`,
		);
		return 1;
	}
}
