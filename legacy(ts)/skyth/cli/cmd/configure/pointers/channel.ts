	...
	if (supportsPairing && hasDeviceToken()) {
		...
		if (useClack) {
			...
			if (isCancel(shouldPair)) {
				return {
					exitCode: 1,
					output: "Pairing is required. Configuration cancelled.",
				};
			}
			doPair = shouldPair;
		} else {
			const answer = (
				await deps.promptInputFn(
					"Pair " +
						channel +
						" now? (REQUIRED - you must send a pairing code from your " +
						channel +
						" chat) (y/n): ",
				)
			)
		...
		if (!doPair) {
			return {
				exitCode: 1,
				output: "Pairing is required for channel configuration.",
			};
		}
		...
		if (doPair) {
			...
			try {
				...
				if (pairingResult.status === "paired" && pairingResult.senderId) {
					...
					if (useClack) {
						clackNote(
							`Pairing timed out or failed. You can pair later with: skyth auth token add-node --channel ${channel}`,
							"Pairing",
						);
					} else {
						console.log(`\nPairing timed out or failed.`);
						console.log(
							`You can pair later with: skyth auth token add-node --channel ${channel}`,
						);
					}
				}
			} finally {
