...
export function resolveDeliveryTarget(params: {
	...
	if (
		explicitChannel &&
		fallback &&
		normalizeChannel(fallback.channel) === normalizeChannel(explicitChannel)
	) {
...
export function loadLastActiveChannelTarget(
...
export function loadAllActiveChannelTargets(
	workspacePath: string,
): Map<string, DeliveryTarget & { ts: number }> {
