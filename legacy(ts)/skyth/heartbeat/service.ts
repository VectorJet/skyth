...
export const HEARTBEAT_PROMPT =
	"Read HEARTBEAT.md in your workspace (if it exists). Follow it strictly. Do not infer or repeat old tasks from prior chats. If nothing needs attention, reply HEARTBEAT_OK.";
	...
	private async runLoop(): Promise<void> {
		while (this.running) {
			await new Promise((resolve) =>
				setTimeout(resolve, this.intervalS * 1000),
			);
