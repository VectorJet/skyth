...
describe("merge router", () => {
	...
	test("detects explicit cross-channel intent", () => {
		expect(
			isExplicitCrossChannelRequest("what was my last message on telegram?"),
		).toBe(true);
		expect(isExplicitCrossChannelRequest("before the switch to discord?")).toBe(
			true,
		);
		expect(
			isExplicitCrossChannelRequest("let us talk about operating systems now"),
		).toBe(false);
	});

	test("short-circuits explicit cross-channel request without provider call", async () => {
		const provider = new MockProvider([
			'{"decision":"separate","confidence":0.1,"reason_code":"x"}',
		]);
		const router = new MergeRouter(provider, "mock/provider");
		const result = await router.classify(
			[],
			[],
			"what was my last message on telegram?",
		);
	...
	test("uses structured LLM JSON output and caches repeat calls", async () => {
		const provider = new MockProvider([
			'{"decision":"continue","confidence":0.91,"reason_code":"cross_channel_reference"}',
		]);
		const router = new MergeRouter(provider, "mock/provider", {
			cacheTtlMs: 60_000,
			cacheMaxEntries: 8,
		...
		const source = [
			{
				role: "user",
				content:
					"this is so confusing but let us talk about operating system now",
			},
		];
		const target = [
			{
				role: "user",
				content: "we switched channels and i am still on exam prep",
			},
		];
		...
		const first = await router.classify(
			source as any,
			target as any,
			"i am still confused about entropy compression",
		);
	...
	test("parses textual key-value router output", async () => {
		...
		const result = await router.classify(
			[
				{ role: "user", content: "we were discussing the migration checklist" },
			] as any,
	...
	test("uses heuristic fallback when provider returns prompt error text", async () => {
		const provider = new MockProvider([
			"Provider error: Invalid prompt: The messages are malformed",
		]);
		const router = new MergeRouter(provider, "mock/provider");
		const result = await router.classify(
			[
				{ role: "assistant", content: "The fix is now live on telegram." },
			] as any,
			[] as any,
			"cool right?",
		);
