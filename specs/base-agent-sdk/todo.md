...
- [x] Create `skyth/base/base_agent/runtime.ts` -- implement runtime orchestration and module wiring
- [x] Create `skyth/base/base_agent/lifecycle.ts` -- init, start, stop, destroy

### 1.2 Context Module (`base_agent/context/`)
- [x] Create stub `builder.ts` with ContextModule class
- [x] Extract `identity.ts` -- identity/persona loading (IDENTITY.md, USER.md, SOUL.md)
- [x] Extract `platform.ts` -- platform output adaptation (Telegram, Discord, CLI, etc.)
- [x] Extract `tone.ts` -- tone mirroring analysis
- [x] Verify context behavior with migration tests (`tests/agent_migration.test.ts`)

### 1.3 Memory Module (`base_agent/memory/`)
- [x] Create stub `store.ts` with MemoryModule class
- [x] Extract consolidation logic from loop.ts -- background consolidation
- [x] Create mental_image.ts -- behavioral observation tracking
- [x] Verify memory read/write still works correctly

### 1.4 Session Module (`base_agent/session/`)
- [x] Create stub `handler.ts` with SessionModule class
- [x] Create `switch_merge.ts` + `cross_channel.ts` -- cross-channel merge routing and merge text/compaction helpers
- [x] Create `bridge.ts` -- sticky bridge continuation from loop.ts
- [x] Verify session lifecycle with migration tests

### 1.5 Delegation Module (`base_agent/delegation/`)
- [x] Create stub `manager.ts` with DelegationModule class
- [x] Implement `call_stack.ts` -- circular call prevention (Phase 2 spec)
- [x] Create `types.ts` -- delegation types
- [x] Test circular call detection (`tests/base_agent_delegation_call_stack.test.ts`)
- [x] Test 2-level nesting enforcement (`tests/base_agent_delegation_call_stack.test.ts`)

### 1.6 Tool Module (`base_agent/tools/`)
- [x] Create stub `registry.ts` with ToolModule class
- [x] Implement `loader.ts` -- file scanner for `*_tool.{ts,py,...}` and `*_tool/` dirs
- [x] Implement `metadata.ts` -- JSDoc-style header metadata parser
- [x] Implement `first_use.ts` -- first-use source code injection for security
- [x] Create `types.ts` -- ToolMetadata, ToolEntry, ToolScope interfaces
- [x] Test auto-discovery with sample tools (`tests/base_agent_tool_discovery.test.ts`)
- [x] Test metadata parsing with various file types (`tests/base_agent_tool_discovery.test.ts`)
- [x] Test first-use review injection (`tests/base_agent_tool_discovery.test.ts`)

### 1.7 Skills Module (`base_agent/skills/`)
- [x] Create stub `loader.ts` with SkillsModule class
- [x] Create `types.ts` -- SkillEntry, SkillMeta interfaces
- [x] Verify skill loading (workspace + builtin), always-on skills, and summary generation

### 1.8 Onboarding Module (`base_agent/onboarding/`)
- [x] Create stub `bootstrap.ts` with OnboardingModule class
- [x] Create `identity_check.ts` -- missing field detection from loop.ts
- [x] Verify onboarding flow remains correct (bootstrap tests in `tests/agent_migration.test.ts`)

### 1.9 Runtime (`base_agent/runtime/`)
- [x] Implement lean agent loop in `runtime/agent_loop_runner.ts`: build messages -> call LLM -> execute tools -> repeat
- [x] Wire modules into runtime via `runtime.ts` + `runtime/message_processor.ts`
- [x] Implement loop detection (repeated tool calls)
- [x] Implement output sanitization (strip think tags, leaked tool calls)
- [x] Implement max iteration guard
- [x] Implement streaming support

### 1.10 Lifecycle (`base_agent/lifecycle.ts`)
- [x] Implement init, start, stop, destroy hooks
- [x] Wire lifecycle wrapper to runtime process flow

### 1.11 Validation
- [x] Run existing tests: `bun test tests/`
- [x] Run typecheck: `bun run typecheck`
- [x] Verify no regressions in agent behavior
...
### 2.1 Directory Setup
- [x] Create `skyth/sdks/agent-sdk/` directory structure
- [x] Create `skyth/sdks/agent-sdk/index.ts` -- public API exports

### 2.2 Core SDK
- [x] Implement `define.ts` -- `defineAgent()` factory function
- [x] Implement `tools.ts` -- `defineTool()` helper function
- [x] Implement `pipeline.ts` -- `definePipeline()` helper function
- [x] Implement `types.ts` -- all SDK types (AgentDefinition, ToolDefinition, etc.)

### 2.3 Manifest & Permissions
- [x] Implement `manifest.ts` -- agent manifest schema validation (extends core/manifest.ts)
- [x] Implement `permissions.ts` -- global tool access resolution

### 2.4 Lifecycle Hooks
- [x] Implement `hooks.ts` -- lifecycle hook types (onInit, onStart, onMessage, etc.)

### 2.5 Provide Agent Namespace
- [x] Implement `Agent.list()` -- list all registered agents
- [x] Implement `Agent.get(id)` -- get agent by ID
- [x] Implement `Agent.Info` interface -- agent metadata type
- [x] Wire into existing `skyth/tools/task.ts` imports from `@/agent/agent`

### 2.6 Validation
- [x] Write unit tests for defineAgent()
- [x] Write unit tests for defineTool()
- [x] Write unit tests for manifest validation
- [x] Write unit tests for permissions resolution
- [x] Run typecheck: `bun run typecheck`
