...
export class MergeRouter {
  ...
  async generateSessionName(messages: SessionMessage[]): Promise<SessionNamingResult> {
    ...
    if (!this.provider) {
      const simpleName = this.generateSimpleName(shortContent);
...
Examples: "Bug fix help", "Code review", "Explain regex", "API design"`,
  ...
  private generateSimpleName(content: string): string {
