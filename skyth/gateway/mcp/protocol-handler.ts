...
import {
	chatGptSearchTool,
	handleChatGptSearch,
} from "@/gateway/meta/tools/search.ts";
import {
	chatGptFetchTool,
	handleChatGptFetch,
} from "@/gateway/meta/tools/tool_fetch.ts";
		...
		try {
			...
			if (rpcMethod === "tools/call") {
				...
				try {
									...
									content: [
										{
											...
											text:
												typeof result === "string"
													? result
													: JSON.stringify(
															pruneGatewayOutputObject(result),
															null,
															2,
														),
										},
					...
					// Return the error to Claude as a tool result with isError:true so
					// the model sees it (per MCP spec) instead of a generic JSON-RPC
					// error that the Anthropic backend masks as
					// "Error occurred during tool execution". Include the tool schema
					// and a nudge so Claude can self-correct.
					const tools = getAllTools();
					const toolDef = tools.get(toolName);

					// If this is a meta wrapper (execute_tool / find_tools / etc.) and
					// the failure is really inside the wrapped tool named in args.tool,
					// surface the wrapped tool's schema instead of the wrapper's.
					...
					const nudgeLines: string[] = [
						`Tool "${effectiveName}" failed: ${errMsg}`,
					];

					// Specifically nudge on missing-required-parameter errors.
					const missingMatch = /Required parameter "([^"]+)" is missing/.exec(
						errMsg,
					);
					if (missingMatch) {
