...
export async function createKeyCommandHandler(args: string[], passedFlags?: Record<string, string | boolean>): Promise<number> {
  ...
  if (!name) {
    ...
    console.log("Usage: skyth auth create-key --name 'my-script' --scopes read,write");
    return 1;
  }
  ...
  for (const scope of scopes) {
    if (!validScopes.includes(scope)) {
      console.error(`Error: invalid scope '${scope}'. Valid scopes: ${validScopes.join(", ")}`);
      return 1;
    }
  }
