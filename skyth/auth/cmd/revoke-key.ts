...
export async function revokeKeyCommandHandler(args: string[], passedFlags?: Record<string, string | boolean>): Promise<number> {
  const { flags, positionals } = passedFlags
    ? { flags: passedFlags, positionals: args }
    : parseArgs(args);
  
  const keyIdOrName = positionals[0] ?? strFlag(flags, "name");
  const force = strFlag(flags, "force") === "true" || strFlag(flags, "f") === "true";

  if (!keyIdOrName) {
    console.error("Error: key id or name is required");
    console.log("Usage: skyth auth revoke-key {key_id | --name 'key-name'} [--force]");
    return 1;
  }
  ...
  console.log(`API key "${key.name}" (${key.key_id}) has been revoked and deleted.`);
