...
export async function createTokenCommandHandler(args: string[], passedFlags?: Record<string, string | boolean>): Promise<number> {
  if (hasDeviceToken()) {
  ...
  try {
    ...
    console.log("Use 'skyth auth token add-node' to register trusted channels.");

    return 0;
  } catch (error) {
    console.error(`Error creating token: ${error instanceof Error ? error.message : error}`);
    return 1;
  }
}
