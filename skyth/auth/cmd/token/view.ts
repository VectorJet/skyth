...
export async function viewTokenCommandHandler(
	args: string[],
	...
	if (filter) {
		const filtered = nodes.filter(
			(n) => n.channel === filter || n.id === filter,
		);
	...
	console.log(
		`KDF: ${tokenInfo.kdf.algorithm} (m=${tokenInfo.kdf.memory_cost}, t=${tokenInfo.kdf.time_cost}, p=${tokenInfo.kdf.parallelism})`,
	);
	console.log(`Encryption: ${tokenInfo.encryption.algorithm}`);
	console.log(
		`Identity Binary: ${hasIdentityBinary() ? "present" : "missing"}`,
	);
	...
	if (nodes.length > 0) {
		...
		for (const node of nodes) {
			...
			console.log(
				`  - ${node.channel} (${node.id}) ${mfa} - trusted since ${node.trusted_at}`,
			);
		}
	}
	...
	console.log(
		"  skyth auth token view device            Device identity factors",
	);
...
async function viewDevice(): Promise<number> {
	...
	const password = await promptPassword(
		"Superuser password (to decrypt device identity): ",
	);
	if (!password) {
	...
	if (!factors) {
		console.error(
			"Error: Failed to decrypt identity factors. Token may have been rotated.",
		);
		return 1;
	}
	...
	const matches = (stored: unknown, live: unknown) =>
		stored === live ? "  " : "!!";
	console.log(
		`Machine ID:     ${maskValue(current.machine_id)} ${matches(factors.machine_id, current.machine_id)}`,
	);
	console.log(
		`Platform:       ${current.platform} ${matches(factors.platform, current.platform)}`,
	);
	console.log(
		`Architecture:   ${current.arch} ${matches(factors.arch, current.arch)}`,
	);
	console.log(
		`OS Type:        ${current.os_type} ${matches(factors.os_type, current.os_type)}`,
	);
	console.log(
		`Kernel:         ${current.kernel} ${matches(factors.kernel, current.kernel)}`,
	);
	console.log(
		`CPU Model:      ${current.cpu_model} ${matches(factors.cpu_model, current.cpu_model)}`,
	);
	console.log(
		`CPU Cores:      ${current.cpu_cores} ${matches(factors.cpu_cores, current.cpu_cores)}`,
	);
	console.log(
		`Total Memory:   ${current.total_memory_gb} GB ${matches(factors.total_memory_gb, current.total_memory_gb)}`,
	);
	console.log(
		`Hostname:       ${current.hostname} ${matches(factors.hostname, current.hostname)}`,
	);
