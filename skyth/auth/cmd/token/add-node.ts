...
const CHANNEL_CREDENTIALS: Record<
	string,
	{ enabled: string; credential: string }
> = {
...
function createChannelAdapter(
	...
	config: any,
	bus: MessageBus,
): (BaseChannel & { setPairingEndpoint(url: string | null): void }) | null {
	switch (channel) {
		case "discord":
			return new DiscordChannel(config, bus);
		case "telegram":
			return new TelegramChannel(config, bus);
		case "slack":
...
function checkChannelConfigured(channel: string): {
	configured: boolean;
	message: string;
} {
	...
	if (!channelConfig) {
		return {
			configured: false,
			message: `Channel '${channel}' is not recognized.`,
		};
	}
	...
	if (!enabled) {
		return {
			configured: false,
			message: `Channel '${channel}' is not enabled. Run: skyth configure channel ${channel}`,
		};
	}
	...
	if (!hasCredential) {
		return {
			configured: false,
			message: `Channel '${channel}' is not configured with credentials. Run: skyth configure channel ${channel}`,
		};
	}
...
export async function addNodeCommandHandler(
	args: string[],
	...
	if (!channel) {
		...
		console.log(
			"Usage: skyth auth token add-node --channel telegram [--timeout 120]",
		);
		return 1;
	}
	...
	try {
		...
		if (adapter) await adapter.stop().catch(() => {});
		console.error(
			`Error during pairing: ${error instanceof Error ? error.message : error}`,
		);
		return 1;
	}
}
