...
export function getHeartbeatPolicy(cfg: Config): HeartbeatPolicy {
  ...
  return {
    ...
    directPolicy: heartbeat.directPolicy ?? DEFAULT_HEARTBEAT_POLICY.directPolicy,
  };
}
...
export function shouldSkipHeartbeat(heartbeatPolicy: HeartbeatPolicy, heartbeatFilePath: string, content: string): boolean {
...
export function suppressHeartbeatDelivery(
  ...
  if (responseText.length <= heartbeatPolicy.ackMaxChars && ackText.startsWith("HEARTBEAT")) {
    return true;
  }
