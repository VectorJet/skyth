...
export async function runSecurityAudit(config: Config): Promise<ReturnType<typeof createSecurityAuditReport>> {
  ...
  if (toolsConfig?.restrict_to_workspace === false) {
    findings.push({
      ...
      detail: "tools.restrict_to_workspace is false - filesystem access is not limited",
      remediation: "Set tools.restrict_to_workspace: true to limit filesystem access",
    });
  }
  ...
  if (agentDefaults) {
    if (!agentDefaults.max_tool_iterations || Number(agentDefaults.max_tool_iterations) > 500) {
      findings.push({
...
export function formatSecurityAuditReport(
  ...
  for (const severity of severityOrder) {
    ...
    const header = severity === "critical" ? "CRITICAL" : severity === "warn" ? "Warnings" : "Info";
    lines.push(`${header}:`);
