...
function resolveTarget(raw: string): { id: string; isPanel: boolean } {
  ...
  for (const prefix of ["mochat:", "group:", "channel:", "panel:"]) {
    if (lower.startsWith(prefix)) {
      ...
      return { id, isPanel: prefix !== "mochat:" || !id.startsWith("session_") };
    }
  }
...
export class MochatChannel extends BaseChannel {
  ...
  async start(): Promise<void> {
    if (!this.config.claw_token) throw new Error("mochat claw_token is required");

    const sessions = Array.isArray(this.config.sessions) ? this.config.sessions.map((v: unknown) => String(v).trim()).filter(Boolean) : [];
    const panels = Array.isArray(this.config.panels) ? this.config.panels.map((v: unknown) => String(v).trim()).filter(Boolean) : [];
  ...
  async send(msg: OutboundMessage): Promise<void> {
    ...
    const parts = [String(msg.content ?? "").trim(), ...((msg.media ?? []).map((x) => String(x).trim()).filter(Boolean))].filter(Boolean);
  ...
  private async refreshTargets(): Promise<void> {
    ...
    if (this.autoPanels) {
      try {
        ...
        const panels = Array.isArray((response as any).panels) ? (response as any).panels : [];
  ...
  private async pollSession(sessionId: string): Promise<void> {
    ...
    const parsedCursor = Number((payload as any).parsedCursor ?? (payload as any).cursor ?? 0);
    ...
    for (const item of messages) {
      ...
      const event = {
        ...
        payload: {
          ...
          content: item?.content ?? item?.messagePlainContent ?? item?.messageSnippet ?? "",
  ...
  private async processInboundEvent(targetId: string, event: Record<string, any>, targetKind: "session" | "panel"): Promise<void> {
    ...
    const rawBody = textValue(payload.content ?? payload.messagePlainContent ?? payload.messageSnippet ?? "").trim();
    if (!author || !rawBody) return;

    const isGroup = targetKind === "panel" || Boolean(payload.groupId);
    const requireMention = isGroup && Boolean(this.config.mention?.require_in_groups);
    const agentUserId = String(this.config.agent_user_id ?? "").trim();
    if (requireMention && agentUserId && !rawBody.includes(`@${agentUserId}`) && !rawBody.includes(`<@${agentUserId}>`)) {
      return;
    }
    ...
    await this.handleMessage(author, targetId, rawBody, [], {
      ...
      sender_name: String(payload?.authorInfo?.nickname ?? payload?.authorInfo?.name ?? "") || undefined,
  ...
  private async postJson(path: string, payload: Record<string, any>): Promise<Record<string, any>> {
