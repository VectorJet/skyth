...
function evaluateSlackPolicy(
	...
	if (isDm) {
		...
		if (
			dmPolicy === "allowlist" &&
			!isSenderAllowed(slackCfg.dm?.allow_from, msg.senderId, "slack")
		) {
	...
	if (isGroup) {
		...
		if (
			groupPolicy === "mention" ||
			slackCfg.groups?.[msg.chatId as string]?.require_mention
		) {
			const mentioned = (msg.metadata?.slack as any)?.mentioned ?? false;
			if (!mentioned) {
...
function evaluateDiscordPolicy(
	cfg: Config,
	msg: InboundMessage,
): InboundPolicyDecision {
	...
	if (isDm) {
		...
		if (
			dmPolicy === "allowlist" &&
			!isSenderAllowed(discordCfg.dm?.allow_from, msg.senderId, "discord")
		) {
...
function evaluateTelegramPolicy(
	cfg: Config,
	msg: InboundMessage,
): InboundPolicyDecision {
	...
	if (isDm) {
		...
		if (
			dmPolicy === "allowlist" &&
			!isSenderAllowed(telegramCfg.dm?.allow_from, msg.senderId, "telegram")
		) {
...
function evaluateWhatsAppPolicy(
	cfg: Config,
	msg: InboundMessage,
): InboundPolicyDecision {
...
export function getChannelPolicyStatus(
	cfg: Config,
	channel: string,
): {
	...
	const channelCfg = cfg.channels as unknown as Record<
		string,
		Record<string, any>
	>;
