...
export class EmailChannel extends BaseChannel {
	...
	constructor(
		...
		this.imapFactory =
			deps?.imapFactory ??
			(() => {
				throw new Error("IMAP factory not configured");
			});
		this.smtpFactory =
			deps?.smtpFactory ??
			(() => {
				throw new Error("SMTP factory not configured");
			});
	}
	...
	async send(msg: OutboundMessage): Promise<void> {
		...
		let subject = this.replySubject(
			this.lastSubjectByChat.get(toAddr) ?? "skyth reply",
		);
		const override = msg.metadata?.subject;
		if (typeof override === "string" && override.trim())
			subject = override.trim();

		const email = {
			from:
				this.emailConfig.from_address ||
				this.emailConfig.smtp_username ||
				this.emailConfig.imap_username,
			to: toAddr,
			subject,
			text: msg.content || "",
		...
		const smtp = this.smtpFactory(
			this.emailConfig.smtp_host,
			this.emailConfig.smtp_port,
			30,
		);
	...
	fetchNewMessages(): Array<Record<string, any>> {
		return this.fetchMessages({
			...
			markSeen: this.emailConfig.mark_seen,
			dedupe: true,
			limit: 0,
		});
	}

	fetchMessagesBetweenDates(
		startDate: Date,
		endDate: Date,
		limit = 20,
		...
		return this.fetchMessages({
			searchCriteria: [
				"SINCE",
				this.formatImapDate(startDate),
				"BEFORE",
				this.formatImapDate(endDate),
			],
			markSeen: false,
			dedupe: false,
	...
	private fetchMessages(params: {
		...
		const client = this.imapFactory(
			this.emailConfig.imap_host,
			this.emailConfig.imap_port,
			this.emailConfig.imap_use_ssl,
		);

		try {
			client.login(
				this.emailConfig.imap_username,
				this.emailConfig.imap_password,
			);
			const [selectStatus] = client.select(
				this.emailConfig.imap_mailbox || "INBOX",
			);
			if (selectStatus !== "OK") return messages;

			const [searchStatus, data] = client.search(
				null,
				...(params.searchCriteria as any),
			);
			if (searchStatus !== "OK" || !data?.length) return messages;

			let ids: Buffer[] = String(data[0])
				.split(" ")
				.filter(Boolean)
				.map((v) => Buffer.from(v));
			if (params.limit > 0 && ids.length > params.limit)
				ids = ids.slice(-params.limit);

			for (const imapId of ids) {
				const [fetchStatus, fetched] = client.fetch(
					imapId,
					"(BODY.PEEK[] UID)",
				);
				...
				const body = (parsed.body || "(empty email body)").slice(
					0,
					this.emailConfig.max_body_chars,
				);
				...
				if (params.dedupe && uid) {
					...
					if (this.processedUids.size > this.maxProcessedUids) {
						this.processedUids = new Set(
							[...this.processedUids].slice(
								Math.floor(this.processedUids.size / 2),
							),
						);
					}
				}
	...
	private extractMessageBytes(fetched: any[]): Buffer | undefined {
		for (const item of fetched) {
			if (Array.isArray(item) && item.length >= 2 && Buffer.isBuffer(item[1]))
				return item[1];
			if (item && typeof item === "object" && Buffer.isBuffer(item[1]))
				return item[1];
		}
		return undefined;
	}
	...
	private parseRawEmail(raw: Buffer): {
		from: string;
		subject: string;
		date: string;
		messageId: string;
	...
	static extractTextBody(rawOrParsed: string): string {
		if (/<html|<p|<br/i.test(rawOrParsed))
			return EmailChannel.htmlToText(rawOrParsed).trim();
