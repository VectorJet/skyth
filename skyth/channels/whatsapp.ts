...
export class WhatsAppChannel extends BaseChannel {
	...
	async start(): Promise<void> {
		if (!this.config.bridge_url)
			throw new Error("whatsapp bridge_url is required");
		this.running = true;
		...
		await new Promise<void>((resolve, reject) => {
			...
			ws.addEventListener("open", () => {
				if (this.config.bridge_token) {
					ws.send(
						JSON.stringify({ type: "auth", token: this.config.bridge_token }),
					);
				}
				this.connected = true;
			...
			ws.addEventListener("error", () =>
				settle(() => reject(new Error("whatsapp websocket error"))),
			);
		});
	}
	...
	private async handleBridgeMessage(
		...
		try {
			...
			console.error(
				`[whatsapp] invalid bridge payload: ${dataText.slice(0, 120)}`,
			);
			return;
		}
		...
		if (msgType === "error") {
			console.error(
				`[whatsapp] bridge error: ${String(data.error ?? "unknown")}`,
			);
			return;
		}
	...
	private sendTypingSignal(chatId: string, action: "start" | "stop"): void {
		...
		this.ws.send(
			JSON.stringify({
				type: "presence",
				to: chatId,
				state: action === "start" ? "composing" : "paused",
			}),
		);
	}
