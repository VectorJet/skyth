import {
	channelsEditCommand,
	requireSuperuserForConfiguredChannel,
} from "@/cli/cmd/channels";
import {
...
import {
	promptInput,
	promptPassword as promptPasswordHelper,
} from "@/cli/cmd/../runtime_helpers";
import { isRedactedBlock } from "@/config/loader/secret-redaction";
...
export async function pairingTelegramCommand(
	...
	const askPassword =
		deps?.promptPasswordFn ??
		(process.stdin.isTTY ? promptPasswordHelper : undefined);
	...
	if (!token) {
		if (!deps?.promptInputFn && !process.stdin.isTTY) {
			return {
				exitCode: 1,
				output:
					"Error: Telegram bot token is required (provide --token in non-interactive mode).",
			};
		}
		const askToken =
			deps?.promptPasswordFn ??
			(process.stdin.isTTY ? promptPasswordHelper : ask);
	...
	const timeoutMs =
		Number.isFinite(args.timeout_ms) && (args.timeout_ms ?? 0) > 0
	...
	emit(
		`Waiting up to ${Math.ceil(timeoutMs / 1000)} seconds for authorization...`,
	);
