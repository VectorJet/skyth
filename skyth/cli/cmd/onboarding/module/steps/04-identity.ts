...
export async function runIdentityStep(ctx: StepContext): Promise<StepResult> {
	...
	const hasSuperuserPassword = hasQuasarAuthRecord();
	...
	if (!shouldConfigure) {
		if (!hasSuperuserPassword) {
			...
			return {
				...
				notices: ["Superuser password will be stored by Quasar."],
				patches: [],
			};
		}
	...
	if (hasSuperuserPassword) {
		...
		try {
			await unlockQuasar(currentPassword.trim() || "");
		} catch {
	...
	const superuserPassword = await clackSecretValue(
		hasSuperuserPassword
			? "New superuser password (leave blank to keep current)"
	...
	if (superuserPasswordChange) {
		notices.push("Superuser password will be stored by Quasar.");
	}
