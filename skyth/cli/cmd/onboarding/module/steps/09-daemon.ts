...
export async function runDaemonStep(ctx: StepContext): Promise<StepResult> {
	...
	const hasSystemd =
		...
		Boolean(process.env.SYSTEMD_EXEC_PID || process.env.INVOCATION_ID);
	...
	if (!hasSystemd) {
		...
		return {
			cancelled: false,
			updates: {},
			notices: ["Systemd not detected, daemon install skipped."],
			patches: [],
		};
	}
	...
	const installChoice = await clackConfirmValue(
