...
async function buildModelOptions(
	...
	const targetProviders: string[] =
		normalizedSelected === "opencode" || normalizedSelected === "opencode_go"
...
export async function runModelSelectionStep(
	ctx: StepContext,
): Promise<StepResult> {
	const {
		clackSelectValue,
	...
	const hasConfiguredAuth = Boolean(
		ctx.cfg.primary_model_provider &&
			(ctx.cfg as any).providers?.[ctx.cfg.primary_model_provider]?.api_key,
	);
	...
	if (hasConfiguredAuth) {
		const selected = await clackSelectValue<"keep" | "skip" | "set">(
			...
			[
				{
					value: "keep",
					label: `Keep current (${ctx.cfg.primary_model_provider || "auto"})`,
				},
	...
	const currentProvider = normalizeProviderID(
		ctx.cfg.primary_model_provider || "openai",
	);
	...
	const modelOptions = await buildModelOptions(
		provider,
		ctx.cfg.primary_model || ctx.cfg.agents?.defaults?.model || "",
	);
	...
	if (modelChoice === MODEL_ENTER_MANUAL) {
		const manualModel = await clackTextValue(
			"Enter model id",
			ctx.cfg.primary_model || "",
		);
		if (manualModel?.trim()) {
