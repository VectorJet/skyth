...
async function runClackFlow(
  ...
  for (const step of steps) {
    ...
    try {
      ...
      if ("installDaemon" in result && typeof result.installDaemon === "boolean") {
      ...
      finalNotices.push(`Step ${step.manifest.name} failed: ${error instanceof Error ? error.message : String(error)}`);
    }
  }

  if (cancelled) {
    return { cancelled: true, mode: finalMode, updates: {}, installDaemon: false, channelPatches: [], notices: [] };
  }
