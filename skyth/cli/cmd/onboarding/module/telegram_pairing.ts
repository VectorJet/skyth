...
export async function waitForTelegramPairing(
	params: WaitForTelegramPairingParams,
	...
	try {
		const bootstrap = await apiCall<TelegramUpdate[]>(
			token,
			"getUpdates",
			{
				offset,
	...
	while (Date.now() < deadline) {
		try {
			...
			const timeoutSeconds = Math.max(
				1,
				Math.min(25, Math.floor(remainingMs / 1000)),
			);
