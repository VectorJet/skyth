...
import { onboardQuasar } from "@/cli/cmd/onboarding/module/quasar_auth";
import type {
	ChannelPatch,
	OnboardingArgs,
	OnboardingDeps,
...
function applyArgsToConfig(cfg: Config, args: OnboardingArgs): void {
	...
	if (args.primary_provider?.trim())
		cfg.primary_model_provider = normalizeProvider(args.primary_provider);
...
function applyChannelPatches(
	cfg: Config,
	patches: ChannelPatch[] | undefined,
): void {
	if (!patches?.length) return;
	const channels = cfg.channels as unknown as Record<
		string,
		Record<string, unknown>
	>;
	for (const patch of patches) {
		const current =
			channels[patch.channel] && typeof channels[patch.channel] === "object"
...
export async function runOnboarding(
	...
	if (interactive) {
		const flow = await runInteractiveFlow(cfg, args, {
			...(deps ?? {}),
	...
	if (args.superuser_password?.trim()) {
		await onboardQuasar(cfg.username, args.superuser_password.trim());
	}
	...
	if (args.no_install_daemon) {
		...
		lines.push(
			"Gateway service install requested. Service installer wiring is not implemented yet.",
		);
	}
