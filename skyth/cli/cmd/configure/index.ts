...
export interface ConfigureDeps {
  ...
  writeSuperuserPasswordRecordFn?: (password: string) => Promise<{ path: string }>;
}
...
export async function configureCommand(
  ...
  const injected = {
    ...
    writeSuperuserPasswordRecordFn: deps?.writeSuperuserPasswordRecordFn ?? (async (p: string) => {
      const { writeSuperuserPasswordRecord } = await import("../../../auth/superuser");
  ...
  try {
    const result = await topicHandler.handler({ args, deps: injected, useClack });
