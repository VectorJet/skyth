...
import type { ConfigureDeps } from "@/cli/cmd/configure/index";
import {
	hasQuasarAuthRecord,
	onboardQuasar,
	unlockQuasar,
...
async function handler({
	...
	const hasExisting = hasQuasarAuthRecord();

	if (hasExisting) {
		const currentPassword = await promptTextValue(
			"Current superuser password",
			deps,
			useClack,
		);
		if (currentPassword === undefined)
			return { exitCode: 1, output: "Cancelled." };
		if (!currentPassword)
			return { exitCode: 1, output: "Error: current password is required." };

		try {
			await unlockQuasar(currentPassword);
		} catch {
			return { exitCode: 1, output: "Error: incorrect current password." };
		}
	}
	...
	if (!newPassword) {
		newPassword =
			(await promptTextValue("New superuser password", deps, useClack)) ?? "";
	}
	if (!newPassword)
		return { exitCode: 1, output: "Error: password cannot be empty." };

	const confirmPassword = await promptTextValue(
	...
	if (newPassword !== confirmPassword)
		return { exitCode: 1, output: "Error: passwords do not match." };

	await onboardQuasar(loadConfig().username, newPassword);
	return {
		exitCode: 0,
		output: "Superuser password updated in Quasar.",
	};
}
