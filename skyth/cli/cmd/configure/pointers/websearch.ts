...
async function handler({
	...
	if (!providerID && !useClack) {
		providerID = (
			await deps.promptInputFn(
				`Web search provider (${WEB_SEARCH_PROVIDERS.map((p) => p.id).join(", ")}): `,
			)
		)
			.trim()
			.toLowerCase();
	...
	if (!validProvider) {
		return {
			exitCode: 1,
			output: `Error: unknown provider '${providerID}'. Available: ${WEB_SEARCH_PROVIDERS.map((p) => p.id).join(", ")}`,
		};
	}
	...
	if (!apiKey && useClack) {
		...
		const input = await deps.promptInputFn(
			`API key for ${validProvider.name}: `,
		);
