import { configureCommand } from "@/cli/commands";
import { boolFlag, strFlag, type ParsedArgs } from "@/cli/runtime_helpers";

export async function configureCommandHandler(parsed: ParsedArgs): Promise<number> {
  const { flags, positionals } = parsed;
  
  const sub = positionals[0];
  const result = await configureCommand({
    topic: sub,
    value: strFlag(flags, "value") ?? positionals[1],
    provider: strFlag(flags, "provider") ?? positionals[1],
    api_key: strFlag(flags, "api_key"),
    api_base: strFlag(flags, "api_base"),
    model: strFlag(flags, "model") ?? positionals[1],
    primary: boolFlag(flags, "primary", false),
    channel: positionals[1],
    enable: boolFlag(flags, "enable", false) || undefined,
    disable: boolFlag(flags, "disable", false) || undefined,
    set: strFlag(flags, "set"),
    json: strFlag(flags, "json"),
  });
  if (result.output) {
    const sink = result.exitCode === 0 ? console.log : console.error;
    sink(result.output);
  }
  return result.exitCode;
}
