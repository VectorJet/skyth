...
export const gatewayHandler: CommandHandler = async ({ positionals, flags }: CommandContext): Promise<number> => {
      ...
      while (running) {
        ...
        try {
          ...
          if (isChannelDeliveryTarget(normalizedMsg.channel)) {
            ...
            channelTargets.set(normalizedMsg.channel, { channel: normalizedMsg.channel, chatId: normalizedMsg.chatId, ts: Date.now() });
            agent.updateChannelTargets(channelTargets);
          }
