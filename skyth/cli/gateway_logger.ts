...
function parseEventLine(message: string): ParsedEvent | null {
	const m = message.match(
		/^\[(event|heartbeat|cron|handoff)\]\[([a-z0-9_-]+)\]\s+([a-z0-9_-]+)(?:\s+([\s\S]*))?$/i,
	);
	if (!m) return null;
	return {
...
function formatMessage(
	...
	if (useColor) {
		...
		decorated = decorated
			.replace(/\[(event|heartbeat|cron|handoff)\]/i, (m) =>
				color(kindColor, m),
			)
...
export function installGatewayLogger(
	options: GatewayLoggerOptions,
): () => void {
