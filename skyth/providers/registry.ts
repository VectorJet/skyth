...
export async function loadModelsDevCatalog(options?: {
	...
	if (!options?.disableFetch) {
		const baseURL =
			options?.url ?? process.env.OPENCODE_MODELS_URL ?? "https://models.dev";
		const fallbackURL =
			options?.fallbackUrl ?? process.env.OPENCODE_MODELS_FALLBACK_URL;
...
export async function listProviderSpecs(options?: {
	...
	const enabled = options?.enabledProviders
		? new Set(options.enabledProviders.map((x) => x.replaceAll("-", "_")))
		: undefined;
	...
	if (options?.includeDynamic !== false) {
		...
		for (const provider of Object.values(catalog)) {
			...
			if (!base.some((p) => p.name === name))
				base.push(dynamicSpecFromProvider(provider));
		}
	}
	...
	return STATIC_PROVIDERS.find((spec) =>
		spec.keywords.some(
			(kw) =>
				...
				modelNormalized.includes(kw.replaceAll("-", "_")),
		),
	);
}
...
export function resolveModelSDKInfo(
	...
	const provider =
		modelsDevCache[providerID] ??
		modelsDevCache[providerID.replaceAll("_", "-")];
		...
		for (const provider of Object.values(catalog)) {
			const modelKey = Object.keys(provider.models).find(
				(k) =>
					...
					k.toLowerCase().endsWith(`/${model.toLowerCase()}`),
			);
			if (modelKey) {
