...
import { MessageTool, type MessageToolSendRecord } from "@/base/base_agent/tools/message";
import { SpawnTool } from "@/base/base_agent/tools/spawn";
import { SubagentManager } from "@/base/base_agent/delegation/manager";
import { CronTool } from "@/base/base_agent/tools/cron";
...
export class AgentLoop {
  ...
  private async initializeRuntimeTools(params: {
    ...
    if (globalToolsAllowed === undefined) {
      ...
      globalToolsAllowed = registry.globalToolsEnabled("base_agent");
    }

    const result = await registerRuntimeTools({
      registry: this.tools as any,
      workspace: this.workspace,
