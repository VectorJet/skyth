...
export class SkillsLoader {
	...
	constructor(workspace: string, builtinSkillsDir?: string) {
		...
		this.builtinSkills =
			builtinSkillsDir ?? resolve(process.cwd(), "skyth", "skills");
	}
	...
	listSkills(filterUnavailable = true): SkillEntry[] {
		...
		return filterUnavailable
			? sorted.filter((item) =>
					this.checkRequirements(this.getSkillMeta(item.name)),
				)
			: sorted;
	}
	...
	getAlwaysSkills(): string[] {
		...
		for (const skill of this.listSkills(true)) {
			...
			if (
				(jsonMeta.always ?? false) ||
				(meta.always ?? "").toLowerCase() === "true"
			) {
				out.push(skill.name);
			}
		}
	...
	private parseSkillJsonMeta(raw: string): Record<string, any> {
		try {
			...
			return d.skyth && typeof d.skyth === "object"
				? d.skyth
				: d.skyth && typeof d.skyth === "object"
					? d.skyth
					: {};
		} catch {
			return {};
	...
	private checkRequirements(skillMeta: Record<string, any>): boolean {
		const requires =
			skillMeta.requires && typeof skillMeta.requires === "object"
				? skillMeta.requires
				: {};
		const bins = Array.isArray((requires as any).bins)
			? (requires as any).bins
			: [];
		const envs = Array.isArray((requires as any).env)
			? (requires as any).env
			: [];
