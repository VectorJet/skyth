...
export class ContextBuilder {
	...
	buildMessages(params: {
		...
		const toneGuide = buildToneAdaptationSection(
			params.history,
		...
		if (!media.length) {
			messages.push({
				role: "user",
				content: `${params.currentMessage}${transitionNote}`,
			});
			return messages;
		}
		...
		content.push({
			type: "text",
			text: `${params.currentMessage}${transitionNote}`,
		});
	...
	addAssistantMessage(
		...
		if (reasoningContent !== undefined)
			next.reasoning_content = reasoningContent;
	...
	private trimToolResultForModel(name: string, result: string): string {
		...
		const maxChars =
			name === "websearch" ? MAX_WEBSEARCH_RESULT_CHARS : MAX_TOOL_RESULT_CHARS;
	...
	private buildKnownProfileSection(files: WorkspaceContextFile[]): string {
		...
		const identity = files.find(
			(file) => file.name.toLowerCase() === "identity.md",
		);
		const hasBootstrap = files.some(
			(file) => file.name.toLowerCase() === "bootstrap.md",
		);

		const userName = extractMarkdownField(user?.content, "Name");
		const userPreferred = extractMarkdownField(
			user?.content,
			"What to call them",
		);
		...
		if (hasBootstrap && (!assistantName || !(userPreferred || userName))) {
			lines.push(
				"- Onboarding is incomplete. Ask only for missing fields, then persist updates immediately.",
			);
		}
		if (hasBootstrap && assistantName && (userPreferred || userName)) {
