export abstract class Tool {
  ...
  private validate(value: any, schema: Record<string, any>, path: string): string[] {
    const t = schema.type;
    const label = path || "parameter";

    if (t && Tool.TYPE_MAP[t]) {
      if (t === "integer" && (!Number.isInteger(value) || typeof value !== "number")) return [`${label} should be integer`];
      if (t === "number" && typeof value !== "number") return [`${label} should be number`];
      if (t === "string" && typeof value !== "string") return [`${label} should be string`];
      if (t === "boolean" && typeof value !== "boolean") return [`${label} should be boolean`];
      if (t === "array" && !Array.isArray(value)) return [`${label} should be array`];
      if (t === "object" && (!value || typeof value !== "object" || Array.isArray(value))) return [`${label} should be object`];
    }
    ...
    if (t === "object" && value && typeof value === "object" && !Array.isArray(value)) {
      ...
      for (const required of schema.required ?? []) {
        if (!(required in value)) errors.push(`missing required ${path ? `${path}.${required}` : required}`);
      }
      for (const [k, v] of Object.entries(value)) {
        if (k in props) errors.push(...this.validate(v, props[k], path ? `${path}.${k}` : k));
      }
    }
    if (t === "array" && Array.isArray(value) && schema.items) {
      value.forEach((item, i) => errors.push(...this.validate(item, schema.items, path ? `${path}[${i}]` : `[${i}]`)));
    }
    return errors;
  }
