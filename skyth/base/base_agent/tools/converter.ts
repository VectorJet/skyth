...
export function isLegacyToolInfoLike(value: unknown): value is LegacyToolInfoLike {
  return isObjectLike(value)
...
function toLegacyContext(context?: Record<string, any>): Record<string, any> {
  return {
    ...
    abort: context?.abort instanceof AbortSignal ? context.abort : new AbortController().signal,
...
export async function convertLegacyToolInfo(value: LegacyToolInfoLike): Promise<ToolDefinition> {
  const initialized = await value.init();
  const name = String(value.id ?? "").trim();
  const description = String(initialized?.description ?? "").trim() || `${name} tool`;
  const parameters = initialized?.parameters ?? { type: "object", properties: {} };
  ...
  return {
    ...
    description,
    parameters,
    async execute(params: Record<string, any>, context?: Record<string, any>): Promise<string> {
      ...
      try {
        ...
        if (isObjectLike(result) && typeof result.output === "string") return result.output;
