...
export class DelegationCallStack {
  ...
  canDelegate(params: {
    ...
    const callerIndex = this.stack.findIndex((item) => item.agentId === params.caller);
    ...
    if (this.stack.some((item) => item.agentId === params.callee)) {
      const earlier = callerIndex >= 0
        ? this.stack.slice(0, callerIndex).some((item) => item.agentId === params.callee)
        : false;
