...
export function waitForConsolidationLock(state: ConsolidationState, key: string): Promise<void> {
  return state.locks.get(key) ?? Promise.resolve();
}

export function setConsolidationLock(state: ConsolidationState, key: string, promise: Promise<void>): void {
  state.locks.set(key, promise);
}

export function clearConsolidationLock(state: ConsolidationState, key: string, promise: Promise<void>): void {
...
export function scheduleConsolidation(params: {
  ...
  if (unconsolidated < state.memoryWindow || state.consolidating.has(session.key)) return;
