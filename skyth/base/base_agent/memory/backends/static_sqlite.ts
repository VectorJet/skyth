...
import { Session } from "@/base/base_agent/session/core/manager";
import type {
	DailySummaryResult,
	MemoryBackend,
	MemoryEventRecord,
	MentalImageObservation,
} from "@/base/base_agent/memory/backend";
...
function sessionPath(sessionsDir: string, sessionKey: string): string {
	return join(
		sessionsDir,
		`${safeFilename(sessionKey.replace(":", "_"))}.jsonl`,
	);
}
...
function formatMentalLine(observation: MentalImageObservation): string {
	const timestamp = new Date(
		observation.timestampMs ?? Date.now(),
	).toISOString();
...
export class StaticSqliteMemoryBackend implements MemoryBackend {
	...
	async consolidate(
		...
		_provider: any,
		_model: string,
		_opts: { archive_all: boolean; memory_window: number },
	): Promise<boolean> {
		try {
	...
	updateMentalImage(observation: MentalImageObservation): void {
		...
		try {
			const current = existsSync(this.mentalImagePath)
				? readFileSync(this.mentalImagePath, "utf-8")
				: "";
	...
	writeDailySummary(date = localDate()): DailySummaryResult {
		...
		if (!this.db) {
			if (!existsSync(target)) {
				writeFileSync(
					target,
					`# Daily Summary ${date}\n\nNo activity (Database unavailable).`,
					"utf-8",
				);
			}
    ...
    `)
		...
		if (!rows.length) {
			...
			for (const row of timeline) {
				...
				sections.push(
					`- ${iso} [${row.kind}][${row.scope}] ${row.action} ${short(row.summary, 80)}`.trim(),
				);
			}
		}
		sections.push("");
