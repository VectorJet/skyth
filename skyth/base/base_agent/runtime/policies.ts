...
export function sanitizeOutput(text: string): { content: string; replyToCurrent: boolean } {
  let out = text;
...
export function shouldForceIdentityToolUse(workspace: string, content: string): boolean {
...
export function isIdentityFileWriteToolCall(name: string, args: Record<string, any>): "user.md" | "identity.md" | null {
