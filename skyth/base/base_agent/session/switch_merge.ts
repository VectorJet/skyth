...
import type { RuntimeContext, RuntimeInbound } from "@/base/base_agent/runtime/types";
import { buildCompactionPrompt, buildCrossChannelMessages } from "@/base/base_agent/session/cross_channel";
...
export async function handlePlatformSwitchMerge(params: {
  ...
  key: string;
  session: Session;
  outboundHandoff?: { sourceKey: string; sourceChannel: string; sourceChatId: string };
}): Promise<{ previousChannel?: string; previousChatId?: string; platformChanged: boolean }> {
  ...
  if (!(platformChanged && previousChannel && previousChatId && runtime.autoMergeOnSwitch)) {
  ...
  if (previousSession.messages.length > 0) {
    const routerResult = outboundHandoff
      ...
      : runtime.stickyBridge.consumeIfActive(previousKey, key, msg.content)
        ...
        : await runtime.mergeRouter.classify(previousSession.messages, session.messages, msg.content);
    ...
    if (routerResult.decision === "continue") {
      const mergeCheck = outboundHandoff
        ? runtime.sessions.shouldMerge(previousKey, key, previousSession, session, 0, 0)
        : runtime.sessions.shouldMerge(previousKey, key, previousSession, session, 0);
      ...
      if (mergeCheck.shouldMerge) {
        ...
        const mergedContent = buildCrossChannelMessages(previousSession.messages, session.messages, previousKey, key);
        ...
        runtime.sessions.graph.merge(previousKey, key, "compact", previousSession.messages.length);
