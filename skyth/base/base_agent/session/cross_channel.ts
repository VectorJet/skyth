...
export function consumePendingMergeIfRequested(params: {
  ...
  if (!isExplicitCrossChannelRequest(params.currentMessage, sourceChannel)) return false;
  ...
  params.sessions.graph.merge(sourceKey, params.targetKey, "compact", sourceSession.messages.length);
  ...
  params.onLog?.(`[session-graph] consumed pending merge ${sourceKey} -> ${params.targetKey} on explicit request`);
  return true;
}
