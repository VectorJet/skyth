...
function isLevelEnabled(level: LogLevel): boolean {
	const envLevel = (
		process.env.SKYTH_LOG_LEVEL ?? "info"
...
export function createSubsystemLogger(subsystem: string): SubsystemLogger {
	return {
		...
		trace: (message, meta) =>
			log("trace", formatMessage(subsystem, "trace", message), meta),
		debug: (message, meta) =>
			log("debug", formatMessage(subsystem, "debug", message), meta),
		info: (message, meta) =>
			log("info", formatMessage(subsystem, "info", message), meta),
		warn: (message, meta) =>
			log("warn", formatMessage(subsystem, "warn", message), meta),
		error: (message, meta) =>
			log("error", formatMessage(subsystem, "error", message), meta),
		fatal: (message, meta) =>
			log("fatal", formatMessage(subsystem, "fatal", message), meta),
