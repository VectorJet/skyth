...
export default defineTool({
  name: "session_merge",
  description: "Manually merge another session's context into the current session. Use this to pull context from another channel into the current conversation.",
  parameters: {
    type: "object",
    properties: {
      source: { type: "string", description: "Source session key to merge from (e.g., 'discord:12345')" },
      mode: { type: "string", enum: ["compact", "full"], description: "Merge mode: 'compact' (summarizes source) or 'full' (includes all messages)" },
    },
    required: ["source"],
  },
