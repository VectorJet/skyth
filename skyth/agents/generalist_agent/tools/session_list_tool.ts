 ...
 * @tags session
 */
import { defineTool } from "@/base/base_agent/sdk/tools";
import type { ToolExecutionContext } from "@/base/base_agent/tools/context";

export default defineTool({
	name: "session_list",
	description:
		"List all sessions with their token counts. Use this to see how much context each channel's session has.",
	parameters: {
		type: "object",
		properties: {},
	},
		...
		for (const { key, branch } of sessions) {
			...
			const mergedFrom =
				branch.mergedFrom.length > 0
					? ` (merged from: ${branch.mergedFrom.join(", ")})`
					: "";
			lines.push(
				`- ${key}: ${msgCount} messages, ~${tokenCount} tokens${mergedFrom}`,
			);
		}
