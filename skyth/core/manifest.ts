...
export function manifestFromObject(
	data: unknown,
	source?: string,
): ModuleManifest {
	if (!data || typeof data !== "object" || Array.isArray(data)) {
		throw new ManifestValidationError(
			fmt(source, "<root>", "manifest root must be an object"),
		);
	}
	...
	for (const key of required) {
		if (!(key in obj)) {
			throw new ManifestValidationError(
				fmt(source, key, "missing required field"),
			);
		}
	}

	const errors: string[] = [];
	const nonEmptyString = (k: string) =>
		typeof obj[k] === "string" && String(obj[k]).trim().length > 0;
	if (!nonEmptyString("id"))
		errors.push(fmt(source, "id", "must be a non-empty string"));
	if (!nonEmptyString("name"))
		errors.push(fmt(source, "name", "must be a non-empty string"));
	if (!nonEmptyString("version"))
		errors.push(fmt(source, "version", "must be a non-empty string"));
	if (!nonEmptyString("entrypoint"))
		errors.push(fmt(source, "entrypoint", "must be a non-empty string"));
	if (
		!Array.isArray(obj.capabilities) ||
		!obj.capabilities.every((v) => typeof v === "string")
	) {
...
export function manifestFromPath(path: string): ModuleManifest {
	try {
		...
		throw new ManifestValidationError(
			fmt(path, "<json>", `invalid JSON: ${message}`),
		);
	}
}
