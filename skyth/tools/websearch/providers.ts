...
export class ExaSearchProvider implements WebSearchProvider {
  ...
  async search(query: string, options: WebSearchOptions): Promise<WebSearchResult> {
    ...
    try {
      const response = await fetch(`${API_CONFIG.EXA.BASE_URL}${API_CONFIG.EXA.ENDPOINT}`, {
        method: "POST",
        headers: {
...
export class SerperSearchProvider implements WebSearchProvider {
  ...
  async search(query: string, options: WebSearchOptions): Promise<WebSearchResult> {
    ...
    try {
      const response = await fetch(`${API_CONFIG.SERPER.BASE_URL}${API_CONFIG.SERPER.SEARCH_ENDPOINT}`, {
        method: "POST",
        headers: {
          "X-API-Key": apiKey,
          "Content-Type": "application/json",
...
export class SerpApiSearchProvider implements WebSearchProvider {
  ...
  async search(query: string, options: WebSearchOptions): Promise<WebSearchResult> {
    ...
    try {
      ...
      const response = await fetch(`${apiBase}${API_CONFIG.SERPAPI.SEARCH_ENDPOINT}?${params}`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
        },
...
export class BraveSearchProvider implements WebSearchProvider {
  ...
  async search(query: string, options: WebSearchOptions): Promise<WebSearchResult> {
    ...
    try {
      const params = new URLSearchParams({
        q: query,
        count: String(options.numResults || API_CONFIG.BRAVE.DEFAULT_NUM_RESULTS),
      })

      const response = await fetch(`${API_CONFIG.BRAVE.BASE_URL}${API_CONFIG.BRAVE.SEARCH_ENDPOINT}?${params}`, {
        method: "GET",
        headers: {
          "X-API-Key": apiKey,
