...
import { BATCH_DESCRIPTION } from "@/tools/descriptions"
...
export const BatchTool = Tool.define("batch", async () => {
  return {
    ...
    async execute(params, ctx) {
      ...
      const { ToolRegistry } = await import("@/registries/tool_registry")
      const registry = new ToolRegistry();
      await registry.autoDiscover(process.cwd());
      
      const availableTools = registry.getDefinitions().map(def => ({ id: def.function.name, ...def.function }));
