...
class BashCompatTool extends Tool {
  ...
  async execute(params: Record<string, any>): Promise<string> {
    ...
    return await this.delegate.execute({ command, working_dir: params.working_dir });
  }
}
...
class WebFetchCompatTool extends Tool {
  ...
  async execute(params: Record<string, any>): Promise<string> {
    return await this.delegate.execute({ url: params.url, extractMode: "text" });
  }
}
...
class TodoWriteTool extends Tool {
  ...
  get description(): string { return "Write the working todo list for this session."; }
  get parameters(): Record<string, any> {
    return {
      type: "object",
      properties: {
...
class TodoReadTool extends Tool {
  ...
  get name(): string { return "todoread"; }
  get description(): string { return "Read the working todo list."; }
