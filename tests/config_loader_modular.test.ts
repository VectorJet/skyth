...
import { getApiKeysPath, getChannelsDirPath, getConfigPath, getMcpConfigFile, getRuntimeConfigPath, loadConfig, saveConfig } from "../skyth/config/loader";

let tempHome = "";
const realHome = process.env.HOME;

beforeEach(() => {
  tempHome = join(process.cwd(), ".tmp", `home-${Date.now()}-${Math.random().toString(16).slice(2)}`);
...
describe("config loader modular", () => {
  test("save and load modular config", () => {
    ...
    expect(loaded.agents.defaults.model).toBe("openrouter/deepseek/deepseek-chat");
  ...
  test("saveConfig does not overwrite existing channel files", () => {
    ...
    writeFileSync(telegramPath, JSON.stringify({
      enabled: true,
