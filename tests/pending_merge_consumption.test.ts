...
describe("pending merge consumption", () => {
	test("consumes pending merge when user explicitly asks cross-channel", async () => {
		...
		try {
			...
			source.addMessage(
				"user",
				"this is so confusing but now lets do operating systems",
			);
			...
			const systemMessages = updated.messages.filter(
				(msg) =>
					msg.role === "system" &&
					String(msg.content ?? "").includes("USER-REQUESTED MERGE"),
			);
	...
	test("auto-merges when cross-channel handoff was initiated by message tool", async () => {
		const workspace = join(
			process.cwd(),
			".tmp",
			`pending-merge-handoff-${Date.now()}`,
		);
		mkdirSync(workspace, { recursive: true });
		try {
			...
			const target = sessions.getOrCreate("discord:99");
			const merged = target.messages.find(
				(msg) =>
					msg.role === "system" &&
					String(msg.content ?? "").includes("CONFIRMED CONTINUATION"),
			);
			expect(Boolean(merged)).toBe(true);
			expect(
				String((merged as Record<string, any>)?._mergeMeta?.sourceKey ?? ""),
			).toBe("telegram:42");
		} finally {
			rmSync(workspace, { recursive: true, force: true });
		}
