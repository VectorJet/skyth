...
function makeRawEmail(fromAddr = "alice@example.com", subject = "Hello", body = "This is the body."): Buffer {
  return Buffer.from([
    `From: <${fromAddr}>`,
    `To: bot@example.com`,
    `Subject: ${subject}`,
...
describe("email channel", () => {
  test("fetch new messages parses unseen and marks seen", () => {
    ...
    class FakeIMAP {
      ...
      login() { return ["OK", ["logged in"]]; }
      select() { return ["OK", ["1"]]; }
      search() { return ["OK", ["1"]]; }
      fetch() { return ["OK", [[Buffer.from("1 (UID 123 BODY[] {200})"), raw], Buffer.from(")")]]; }
      store(id: Buffer, op: string, flags: string) { this.storeCalls.push([id, op, flags]); return ["OK", [""]]; }
      logout() { return ["BYE", [""]]; }
    }
  ...
  test("start returns immediately without consent", async () => {
    ...
    const channel = new EmailChannel(cfg, new MessageBus(), {
      imapFactory: () => ({ login() {}, select() { return ["OK", [""]]; }, search() { return ["OK", [""]]; }, fetch() { return ["OK", []]; }, store() {}, logout() {} } as any),
      smtpFactory: () => ({ starttls() {}, login() {}, sendMessage() {} }),
    });
  ...
  test("send skips when auto reply disabled", async () => {
    ...
    const channel = new EmailChannel(cfg, new MessageBus(), {
      imapFactory: () => ({ login() {}, select() { return ["OK", [""]]; }, search() { return ["OK", [""]]; }, fetch() { return ["OK", []]; }, store() {}, logout() {} } as any),
      smtpFactory: () => ({ starttls() {}, login() {}, sendMessage(msg: any) { sent.push(msg); } }),
    });

    await channel.send({ channel: "email", chatId: "alice@example.com", content: "Should not send." });
    expect(sent).toEqual([]);

    await channel.send({ channel: "email", chatId: "alice@example.com", content: "Force send.", metadata: { force_send: true } });
    expect(sent.length).toBe(1);
  });
  ...
  test("send skips when consent not granted", async () => {
    ...
    const channel = new EmailChannel(cfg, new MessageBus(), {
      imapFactory: () => ({ login() {}, select() { return ["OK", [""]]; }, search() { return ["OK", [""]]; }, fetch() { return ["OK", []]; }, store() {}, logout() {} } as any),
      smtpFactory: () => ({ starttls() {}, login() {}, sendMessage() { called = true; } }),
    });

    await channel.send({ channel: "email", chatId: "alice@example.com", content: "Should not send.", metadata: { force_send: true } });
  ...
  test("fetch messages between dates uses since before without mark seen", () => {
    ...
    class FakeIMAP {
      ...
      login() { return ["OK", ["logged in"]]; }
      select() { return ["OK", ["1"]]; }
      search(...args: any[]) { this.searchArgs = args; return ["OK", ["5"]]; }
      fetch() { return ["OK", [[Buffer.from("5 (UID 999 BODY[] {200})"), raw], Buffer.from(")")]]; }
      store(id: Buffer, op: string, flags: string) { this.storeCalls.push([id, op, flags]); return ["OK", [""]]; }
      logout() { return ["BYE", [""]]; }
    }
