...
describe("migrate command", () => {
  test("migrates from openclaw into skyth", async () => {
    ...
    try {
      ...
      writeFileSync(join(openclawWorkspace, "AGENTS.md"), "OpenClaw agent rules", "utf-8");
      writeFileSync(join(openclawWorkspace, "memory", "2026-02-24.md"), "# Daily note", "utf-8");
      writeFileSync(join(openclawWorkspace, "memory", "heartbeat-state.json"), JSON.stringify({ last_ok_at: 123 }, null, 2), "utf-8");
      ...
      writeFileSync(join(openclawSessions, "sess-1.jsonl"), `${sessionLines.join("\n")}\n`, "utf-8");
      writeFileSync(join(openclawSessions, "sessions.json"), JSON.stringify({
        "agent:main:main": {
          sessionId: "sess-1",
          deliveryContext: { channel: "telegram", to: "telegram:7405495226" },
        },
      }, null, 2), "utf-8");

      writeFileSync(join(openclaw, "cron", "jobs.json"), JSON.stringify({
        version: 1,
        jobs: [
          {
            id: "job-1",
            ...
            schedule: { kind: "every", everyMs: 3_600_000, anchorMs: 1771927200000 },
      ...
      writeFileSync(join(openclaw, "credentials", "telegram-allowFrom.json"), JSON.stringify({
        version: 1,
        allowFrom: ["7405495226"],
      }, null, 2), "utf-8");

      writeFileSync(join(openclaw, "openclaw.json"), JSON.stringify({
        agents: {
          defaults: {
      ...
      const result = await migrateCommand({ direction: "from", target: "openclaw" });
      ...
      expect(existsSync(join(skythWorkspace, "memory", "daily", "2026-02-24.md"))).toBeTrue();
      expect(existsSync(join(skythWorkspace, "memory", "heartbeat-state.json"))).toBeTrue();

      const migratedSession = join(skythWorkspace, "sessions", "telegram_7405495226.jsonl");
      expect(existsSync(migratedSession)).toBeTrue();
      const sessionRows = readFileSync(migratedSession, "utf-8").trim().split("\n");
      const metadata = JSON.parse(sessionRows[0]!);
      expect(metadata._type).toBe("metadata");
      expect(metadata.key).toBe("telegram:7405495226");

      const cronStore = JSON.parse(readFileSync(join(env.home, ".skyth", "cron", "jobs.json"), "utf-8"));
      expect(cronStore.jobs[0].schedule.every_ms).toBe(3_600_000);
      expect(cronStore.jobs[0].payload.message).toBe("ping");

      const telegramCfg = JSON.parse(readFileSync(join(env.home, ".skyth", "channels", "telegram.json"), "utf-8"));
  ...
  test("migrates from skyth into openclaw", async () => {
    ...
    try {
      ...
      writeFileSync(join(skythWorkspace, "AGENTS.md"), "Skyth agent rules", "utf-8");
      writeFileSync(join(skythWorkspace, "memory", "daily", "2026-02-25.md"), "# Skyth day", "utf-8");
      writeFileSync(join(skythWorkspace, "memory", "heartbeat-state.json"), JSON.stringify({ last_ok_at: 456 }, null, 2), "utf-8");
      writeFileSync(join(skythWorkspace, "sessions", "telegram_7405495226.jsonl"), [
        JSON.stringify({
          _type: "metadata",
        ...
        JSON.stringify({ role: "user", content: "hello skyth", timestamp: "2026-02-25T08:00:30.000Z" }),
      ].join("\n") + "\n", "utf-8");

      writeFileSync(join(skyth, "cron", "jobs.json"), JSON.stringify({
        version: 1,
        jobs: [
          {
            id: "cron-1",
            ...
            payload: { kind: "daily_summary", message: "2026-02-25", deliver: false },
      ...
      writeFileSync(join(env.home, ".openclaw", "openclaw.json"), JSON.stringify({
        channels: { telegram: {} },
        agents: { defaults: { model: { primary: "" } } },
      }, null, 2), "utf-8");

      const result = await migrateCommand({ direction: "to", target: "openclaw" });
      expect(result.exitCode).toBe(0);
      ...
      expect(existsSync(join(openclawWorkspace, "memory", "2026-02-25.md"))).toBeTrue();
      expect(existsSync(join(openclawWorkspace, "memory", "heartbeat-state.json"))).toBeTrue();

      const openclawCron = JSON.parse(readFileSync(join(env.home, ".openclaw", "cron", "jobs.json"), "utf-8"));
      expect(openclawCron.jobs[0].schedule.expr).toBe("55 23 * * *");
      expect(openclawCron.jobs[0].payload.kind).toBe("systemEvent");

      const allow = JSON.parse(readFileSync(join(env.home, ".openclaw", "credentials", "telegram-allowFrom.json"), "utf-8"));
      expect(allow.allowFrom).toEqual(["7405495226"]);

      const ocCfg = JSON.parse(readFileSync(join(env.home, ".openclaw", "openclaw.json"), "utf-8"));
      expect(ocCfg.channels.telegram.botToken).toBe("99999:telegram-token");
      expect(ocCfg.agents.defaults.model.primary).toBe("groq/moonshotai/kimi-k2-instruct-0905");

      const sessionsDir = join(env.home, ".openclaw", "agents", "main", "sessions");
      const created = readdirSync(sessionsDir).filter((file) => file.endsWith(".jsonl"));
