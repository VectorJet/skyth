...
function createTempHome(name: string): string {
  const base = join(process.cwd(), ".tmp", `${name}-${Date.now()}-${Math.floor(Math.random() * 10000)}`);
...
function createDeviceTokenMarker(home: string): void {
  ...
  mkdirSync(join(home, ".skyth", "auth", "device", "identity"), { recursive: true });
  writeFileSync(tokenPath, JSON.stringify({ kind: "device_identity" }), "utf-8");
}
