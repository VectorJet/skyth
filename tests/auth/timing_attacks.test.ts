...
describe("Security: Constant-Time Comparisons", () => {
	describe("JWT Signature Verification", () => {
		...
		test("timing differences between early and late byte mismatches should be statistically negligible", () => {
			...
			const failFirstByte = validSig.replace(
				/^./,
				validSig[0] === "a" ? "b" : "a",
			);
			...
			const failLastByte = validSig.replace(
				/.$/,
			...
			for (let i = 0; i < 500; i++) {
				try {
					verifyJWT(earlyFailToken);
				} catch {}
				try {
					verifyJWT(lateFailToken);
				} catch {}
			}
			...
			const diffPercent =
				Math.abs(durationEarly - durationLate) /
				Math.max(durationEarly, durationLate);
