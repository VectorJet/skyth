...
describe("base agent tool discovery", () => {
  ...
  test("loader discovers *_tool files and parses metadata", () => {
    ...
    writeFileSync(join(root, "nested", "ignore.ts"), "export default {}", "utf-8");
  ...
  test("global tools follow *_tool naming and include metadata headers", () => {
    const entries = loadToolEntries(join(process.cwd(), "skyth", "tools"), "global");
    const byName = new Map(entries.map((entry) => [entry.metadata.name, entry]));
