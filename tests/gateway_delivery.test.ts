...
function writeSessionMetadata(workspace: string, file: string, meta: {
  updatedAt: string;
  lastChannel: string;
  lastChatId: string;
}): void {
  ...
  writeFileSync(join(sessionsDir, file), `${JSON.stringify(payload)}\n`, "utf-8");
}
...
describe("gateway delivery target resolution", () => {
  ...
  test("loads latest non-system target from session metadata", () => {
    const workspace = join(process.cwd(), ".tmp", `gateway-target-${Date.now()}`);
