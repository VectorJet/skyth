...
class RecoverAfterProviderError extends LLMProvider {
  ...
  async chat(_params: {
    ...
    if (this.calls === 1) {
      return {
        content: "Provider error: Failed after 3 attempts. Last error: temporary upstream issue",
        tool_calls: [],
        finish_reason: "stop",
      };
    }
...
describe("base agent failsafe", () => {
  test("recovers from transient provider error and continues turn", async () => {
    const workspace = join(process.cwd(), ".tmp", `failsafe-recover-${Date.now()}`);
  ...
  test("uses tool-result fallback when provider fails after tool execution", async () => {
    const workspace = join(process.cwd(), ".tmp", `failsafe-tool-${Date.now()}`);
    mkdirSync(workspace, { recursive: true });
    try {
      const loop = new AgentLoop({
        bus: new MessageBus(),
  ...
  test("degrades gracefully on persistent provider failure without surfacing raw provider error", async () => {
    const workspace = join(process.cwd(), ".tmp", `failsafe-persistent-${Date.now()}`);
    mkdirSync(workspace, { recursive: true });
    try {
      const loop = new AgentLoop({
        bus: new MessageBus(),
