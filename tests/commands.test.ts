...
mock.module("@clack/prompts", () => ({
	...
	autocomplete: async (opts: any) =>
		opts.initialValue ?? opts.options?.[0]?.value,
}));
import {
	channelsEditCommand,
	initAlias,
	pairingTelegramCommand,
	runOnboarding,
} from "../skyth/cli/commands";
...
describe("commands and provider matching", () => {
	test("run onboarding non interactive", async () => {
		...
		for (const file of [
			...
			const expected = stripFrontMatter(
				readFileSync(join(templateDir, file), "utf-8"),
			);
		...
		const output = await initAlias(
			{
				...
				primary_model: "anthropic/claude-sonnet-4-0",
			},
			{
				configPath: join(base, "config.yml"),
	...
	test("run onboarding stores superuser password in auth jsonl without plaintext", async () => {
		...
		const hashFile = join(
			...
			"superuser",
			"hashes",
			"superuser_password.jsonl",
		);
		expect(existsSync(hashFile)).toBeTrue();
		const lines = readFileSync(hashFile, "utf-8")
			.split("\n")
	...
	test("openai codex strip prefix supports hyphen and underscore", () => {
		expect(stripModelPrefix("openai-codex/gpt-5.1-codex")).toBe(
			"gpt-5.1-codex",
		);
		expect(stripModelPrefix("openai_codex/gpt-5.1-codex")).toBe(
			"gpt-5.1-codex",
		);
	});
	...
	test("channels edit command writes channel config", () => {
		...
		const raw = JSON.parse(
			readFileSync(join(channelsDir, "telegram.json"), "utf-8"),
		);
	...
	test("pairing telegram command captures /start code and appends allowlist", async () => {
		...
		const fetchImpl: typeof fetch = (async (input, init) => {
			...
			if (method === "sendMessage") {
				return new Response(
					JSON.stringify({ ok: true, result: { message_id: 9, payload } }),
					{ status: 200 },
				);
			}

			return new Response(
				JSON.stringify({ ok: false, description: "unexpected method" }),
				{ status: 500 },
			);
		}) as any;
