...
describe("tool validation", () => {
	...
	test("type and range", () => {
		...
		expect(
			tool
				.validateParams({ query: "hi", count: 0 })
				.some((e) => e.includes("count must be >= 1")),
		).toBeTrue();
		expect(
			tool
				.validateParams({ query: "hi", count: "2" })
				.some((e) => e.includes("count should be integer")),
		).toBeTrue();
	});
	...
	test("enum and min length", () => {
		...
		expect(
			errors.some((e) => e.includes("query must be at least 2 chars")),
		).toBeTrue();
	...
	test("nested object and array", () => {
		...
		expect(
			errors.some((e) => e.includes("missing required meta.tag")),
		).toBeTrue();
		expect(
			errors.some((e) => e.includes("meta.flags[0] should be string")),
		).toBeTrue();
	});
