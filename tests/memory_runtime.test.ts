...
describe("memory runtime", () => {
  ...
  test("memory store writes daily summary and session primer", () => {
    ...
    memory.recordEvent({ kind: "event", scope: "agent", action: "send", summary: "hello", timestamp_ms: ts });
    memory.recordEvent({ kind: "event", scope: "telegram", action: "receive", summary: "yo", timestamp_ms: ts + 1 });
